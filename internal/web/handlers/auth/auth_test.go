...
type authTestOptions struct {
	...
	// EnforceSavedRepliesUnlimited flips the PRO-EXT01-07a enforce knob
	// so tests can pin the hard-block behaviour separately from the
...
func authTemplatesFS() fs.FS {
	...
	//nolint:gosec // G101 false positive: test fixture, not a hardcoded credential.
	tokensTpl := `{{ define "page" }}<form>{{ with .CreateError }}<p class=error>{{.}}</p>{{ end }}<input name=csrf_token value="{{.CSRFToken}}">{{ if .JustCreatedRaw }}RAW={{.JustCreatedRaw}}{{ end }}FINEGRAINED={{.FineGrainedAllowed}};REPOCHOICES={{ range .RepoChoices }}{{.ID}}:{{.Name}};{{ end }}TOKENS={{ range .Tokens }}{{.ID}}:{{.TokenPrefix}}{{ if .RepoID.Valid }}:bound={{.RepoID.Int64}}{{ end }}{{ if .RevokedAt.Valid }}:revoked{{ end }};{{ end }}</form>{{ end }}`
