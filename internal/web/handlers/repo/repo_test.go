...
func minimalTemplatesFS() fstest.MapFS {
	...
	return fstest.MapFS{
		...
		"repo/security_secret_scanning.html": {Data: []byte(`{{ define "page" }}BYPASS={{ .BypassAllowed }};{{ with .Error }}ERROR={{ . }};{{ end }}{{ with .Success }}SUCCESS={{ . }};{{ end }}{{ range .Findings }}FINDING={{ .Pattern }}:{{ .Path }}:{{ .Status }};{{ end }}{{ range .BypassRequests }}REQ={{ .Pattern }}:{{ .Path }}:{{ .CommitOid }}:{{ .LineNo }}:{{ .Status }};{{ end }}{{ range .Allowlist }}ALLOW={{ .Pattern }}:{{ .Path }};{{ end }}{{ end }}`)},
		"repo/security_sbom.html":            {Data: []byte(`{{ define "page" }}GATE={{ .Gate.Allowed }};SNAP={{ .HasSnapshot }};CAN={{ .CanGenerate }};GEN={{ .Generated }};{{ with .LatestExport }}EXPORT={{ .ShortHeadSHA }}:{{ .ByteCount }}:{{ .Stale }};{{ end }}{{ range .Dependencies }}DEP={{ .Ecosystem }}:{{ .PackageName }}:{{ .PackageVersion }};{{ end }}{{ with .Error }}ERROR={{ . }};{{ end }}{{ end }}`)},
		"repo/security_attestations.html":    {Data: []byte(`{{ define "page" }}GATE={{ .Gate.Allowed }};CAN={{ .CanUpload }};CREATED={{ .Created }};{{ with .Error }}ERROR={{ . }};{{ end }}{{ range .Attestations }}ATT={{ .ID }}:{{ .SubjectName }}:{{ .ShortDigest }}:{{ .PredicateType }};{{ end }}{{ end }}`)},
		...
		"repo/security_advisory_detail.html": {Data: []byte(`{{ define "page" }}ADV={{ .Advisory.Row.Identifier }}:{{ .Advisory.Row.State }};DESC={{ .Advisory.RenderedDescription }};COLLABS={{ len .Collaborators }};{{ with .Error }}ERROR={{ . }};{{ end }}{{ range .Collaborators }}COLLAB={{ .Kind }}:{{ .Name }}:{{ .Role }};{{ end }}{{ range .Events }}EVENT={{ .EventType }}:{{ .OldState }}>{{ .NewState }};{{ end }}{{ end }}`)},
