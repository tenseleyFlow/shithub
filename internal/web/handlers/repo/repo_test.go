...
func minimalTemplatesFS() fstest.MapFS {
	...
	return fstest.MapFS{
		...
		"repo/settings_environments.html": {Data: []byte(`{{ define "page" }}{{ with .Error }}ERROR={{ . }};{{ end }}{{ range .Environments }}ENV={{ .Name }}:{{ .DeploymentBranchPolicy }}:{{ .WaitTimerMinutes }}:{{ .SecretCount }};{{ range .BranchPatterns }}PATTERN={{ . }};{{ end }}{{ end }}{{ with .Selected }}SELECTED={{ .Name }}:{{ .DeploymentBranchPolicy }}:{{ .WaitTimerMinutes }};{{ range .Secrets }}SECRET={{ .Name }};{{ end }}{{ end }}{{ end }}`)},
		...
		"repo/pull_view.html":             {Data: []byte(`{{ define "page" }}STATE={{ .PullStats.CheckState }};SUMMARY={{ if .PullStats.CheckSummary.Show }}{{ .PullStats.CheckSummary.StateClass }}:{{ .PullStats.CheckSummary.Label }}:{{ .PullStats.CheckSummary.Href }}{{ end }};REQ={{ .RequiredChecks.HasRequired }}:{{ .RequiredChecks.Satisfied }}:{{ range .RequiredChecks.Missing }}{{ . }}|{{ end }};{{ range .CheckGroups }}{{ range .Runs }}RUN={{ .AppSlug }}:{{ .R.Name }}:{{ .StateClass }}:{{ .DetailsHref }}:{{ .CanRerun }}:{{ .RerunHref }};{{ end }}{{ end }}{{ end }}`)},
